...
def main(args):
    ...
    model = ECDGDST(model_config, len(op2id), len(domain2id), op2id['update'])
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            domain_scores, state_scores, gen_scores = model(input_ids=input_ids,
                                                            ...
                                                            teacher=teacher)
            temp1 = state_scores.view(-1, len(op2id))
            temp2 = op_ids.view(-1)
            temp3 = temp1
            temp4 = temp2
    ...
    model = ECDGDST(model_config, len(op2id), len(domain2id), op2id['update'], args.exclude_domain)
