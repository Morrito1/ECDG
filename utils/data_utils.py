    ...
    for dial_dict in dials:
        ...
        for ti, turn in enumerate(dial_dict["dialogue"]):
            ...
            op_labels, generate_y, gold_state = make_turn_label(slot_meta, last_dialog_state,
                                                                turn_dialog_state,
                                                                tokenizer, op_code)
            if (ti + 1) == len(dial_dict["dialogue"]):#判断是不是最后一轮
                is_last_turn = True
            else:
                is_last_turn = False
...
class TrainingInstance:
    ...
    def make_instance(self, tokenizer, max_seq_length=None,
                      word_dropout=0., slot_token='[SLOT]'):    #生成ID列表，存在self的属性里面
        ...
        for s in self.slot_meta:
            ...
            if v is not None:
                ...
                t.extend(['-', '[NULL]'])
            state.extend(t)
        avail_length_1 = max_seq_length - len(state) - 3  #减三没懂是什么意思，应该是【CLS】、【SEP】、【SEP】的占位
        ...
        if len(diag_1) > avail_length:  # truncated
            avail_length = len(diag_1) - avail_length     #如果diag1的长度太长就截取后面一段？？为什么不是截取剩余长度呢？
            diag_1 = diag_1[avail_length:]

        if len(diag_1) == 0 and len(diag_2) > avail_length_1:
            avail_length = len(diag_2) - avail_length_1
            diag_2 = diag_2[avail_length:]     #如果diag1长度为0并且diag2长度过于长，就也截取后面一段。
        ...
        if word_dropout > 0.:
            ...
            word_drop = np.random.binomial(drop_mask.astype('int32'), word_dropout)#int64改为了int32
            diag = [w if word_drop[i] == 0 else '[UNK]' for i, w in enumerate(diag)]
        input_ = diag + state      #将两轮对话和上一轮次的状态拼接
        segment = segment + [1]*len(state)     #segment拼接上state的segment
        self.input_ = input_

        self.segment_id = segment
        slot_position = []
        for i, t in enumerate(self.input_):    #找出所有slot所在的位置
            if t == slot_token:
                slot_position.append(i)
        self.slot_position = slot_position

        input_mask = [1] * len(self.input_)    #真正的输入的mask,与后面填充的区分开
        self.input_id = tokenizer.convert_tokens_to_ids(self.input_)
        if len(input_mask) < max_seq_length:     #补足长度为256
